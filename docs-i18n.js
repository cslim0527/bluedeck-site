...
window.I18N_EXTRA = {
  en: {
    ...
    "docs.meta.desc": "BlueDeck documentation: install, workspaces, the terminal grid, agents, command palette, Agent Hub (MCP), git worktree isolation, diff review, bug reporting, licensing and updates.",
    ...
    "docs.palette.t": "Command palette",
    "docs.palette.b": "<p>Press <kbd>⌘K</kbd> (<kbd>Ctrl+Alt+K</kbd> on Windows) to open the <b>command palette</b>. Type to filter, then hit Enter:</p><ul><li><b>Jump to any terminal</b> in any workspace — entries show each pane's shortcut, agent and waiting status.</li><li><b>Switch workspaces</b>, open the <b>diff review</b>, create a <b>new terminal or workspace</b>, toggle the sidebar, open settings, or report a bug.</li></ul><p>Escape closes the palette and puts focus right back in the terminal you were in.</p>",
    "docs.status.t": "Agent status awareness",
    "docs.status.b": "<p>BlueDeck watches each pane's PTY output and screen patterns to track it as one of:</p><ul><li><b style='color:var(--green)'>working</b> — the agent is actively doing something</li><li><b style='color:var(--amber)'>needs-input</b> — it's waiting on you</li><li><b style='color:var(--ink-3)'>idle</b> — nothing running</li></ul><p>Status shows as a colored dot + label per pane and as red/amber badges per workspace in the sidebar. When the app is in the background, <b>macOS notifications</b> ping you the moment an agent needs a hand.</p><p>When agents are stuck waiting, a red <b>&ldquo;N waiting&rdquo;</b> pill appears in the topbar. Press <kbd>⌘J</kbd> (<kbd>Ctrl+Alt+J</kbd> on Windows) — or click the pill, or a workspace's red badge — to jump straight to the next waiting terminal, switching workspaces if needed.</p><p>Notifications are configurable in <b>Settings</b> — needs-input + turn done (default), needs-input only, or off. Clicking a needs-input notification brings the app forward and jumps straight to that terminal.</p>",
    ...
    "docs.shortcuts.b": "<p>Press <kbd>⌘/</kbd> (<kbd>Ctrl+Alt+/</kbd> on Windows) inside the app to see this list any time.</p><table><thead><tr><th>Action</th><th>macOS</th><th>Windows</th></tr></thead><tbody><tr><td>Focus terminal 1–9</td><td><kbd>⌘1</kbd>–<kbd>⌘9</kbd></td><td><kbd>Ctrl+Alt+1</kbd>–<kbd>9</kbd></td></tr><tr><td>Jump to the next terminal waiting for input</td><td><kbd>⌘J</kbd></td><td><kbd>Ctrl+Alt+J</kbd></td></tr><tr><td>Command palette</td><td><kbd>⌘K</kbd></td><td><kbd>Ctrl+Alt+K</kbd></td></tr><tr><td>Toggle sidebar</td><td><kbd>⌘B</kbd></td><td><kbd>Ctrl+B</kbd></td></tr><tr><td>Shortcut cheat sheet</td><td><kbd>⌘/</kbd></td><td><kbd>Ctrl+Alt+/</kbd></td></tr><tr><td>Rename a terminal / workspace</td><td colspan='2'>Double-click its name</td></tr></tbody></table>",

    "docs.license.t": "Licensing & Lite mode",
    "docs.license.b": "<div class='callout'><span class='ci'>🎉</span><span><b>Public beta:</b> every feature is free through <b>Aug 31, 2026</b> — no key needed. When the beta ends, a fresh 14-day trial starts for everyone. The plans below apply at the official launch.</span></div><p>Every install starts with a <b>14-day full trial</b> (all features, no credit card). When the trial ends, BlueDeck drops to <b>Lite mode</b>:</p><ul><li><b>Lite (free):</b> the core terminal — 1 workspace × 2 terminals, agent launch, IME, prompt bar and layout restore.</li><li><b>Pro (license):</b> the full multi-agent deck — status awareness, worktree isolation, diff review, Agent Hub, broadcast, auto-approve and conversation resume.</li></ul><p>Activate a device in <b>Settings → License</b> by entering your key. Activation is stored and the app then runs <b>offline indefinitely</b>; it revalidates online in the background and frees the seat when you &ldquo;Deactivate this device&rdquo;. Paid features are enforced in the Rust core, not just the UI.</p>",
    ...
    "docs.bugs.t": "Reporting bugs",
    "docs.bugs.b": "<p>Found a problem? File it without leaving the app: open <b>Settings → Support → Report a bug</b>, or run it from the command palette (<kbd>⌘K</kbd>).</p><ul><li>Write a title and description; optionally attach <b>diagnostics</b> (version, OS, license tier, workspace/terminal counts) — previewed exactly as it will be sent. Terminal contents are never included.</li><li>The first submit connects your GitHub account with a one-time <b>device code</b>; after that it's one click. Reports are filed as <b>GitHub issues under your account</b> on <code>cslim0527/blue-deck-releases</code>, so we can follow up with you on the thread.</li><li>Prefer not to connect? <b>&ldquo;Open in browser&rdquo;</b> pre-fills the issue on github.com instead.</li></ul>",
    "docs.faq.t": "FAQ",
    "docs.faq.b": "<h3>Is BlueDeck free right now?</h3><p>Yes — BlueDeck is in <b>public beta</b> and every feature is free through <b>Aug 31, 2026</b>, no key needed. When the beta ends, a fresh 14-day trial starts for everyone and Pro pricing goes live.</p><h3>Which agents are supported?</h3><p>Claude Code, Codex, Gemini CLI and Aider are auto-detected. Any other CLI can be run by hand in a pane.</p><h3>Is my code sent anywhere?</h3><p>No. Terminals run locally on your machine, and the Agent Hub MCP server binds to <code>127.0.0.1</code> (localhost) only. Agents talk to their own providers exactly as they would in a normal terminal.</p><h3>Does it work offline?</h3><p>Yes. Once a license is activated it's stored locally and the app runs offline; it only revalidates online in the background when it can.</p><h3>Which platforms are supported?</h3><p>macOS on Apple Silicon and Windows on x64. Both auto-update.</p>",
  },
  ko: {
    "docs.meta.title": "BlueDeck 문서 — One deck. Every agent.",
    "docs.meta.desc": "BlueDeck 문서: 설치, 워크스페이스, 터미널 그리드, 에이전트, 커맨드 팔레트, Agent Hub(MCP), git worktree 격리, diff 리뷰, 버그 신고, 라이선스와 업데이트.",
    "docs.hero.eyebrow": "Documentation",
    ...
    "docs.palette.t": "커맨드 팔레트",
    "docs.palette.b": "<p><kbd>⌘K</kbd>(Windows는 <kbd>Ctrl+Alt+K</kbd>)로 <b>커맨드 팔레트</b>를 엽니다. 입력해서 필터링하고 Enter로 실행합니다:</p><ul><li><b>모든 워크스페이스의 터미널로 즉시 이동</b> — 항목에 단축키·에이전트·대기 상태가 함께 표시됩니다.</li><li><b>워크스페이스 전환</b>, <b>diff 리뷰</b> 열기, <b>새 터미널/워크스페이스</b> 만들기, 사이드바 토글, 설정, 버그 신고까지 한 곳에서.</li></ul><p>Escape로 닫으면 포커스가 원래 터미널로 돌아갑니다.</p>",
    "docs.status.t": "에이전트 상태 인식",
    "docs.status.b": "<p>BlueDeck는 각 패널의 PTY 출력과 화면 패턴을 관찰해 다음 중 하나로 추적합니다:</p><ul><li><b style='color:var(--green)'>working</b> — 에이전트가 작업 중</li><li><b style='color:var(--amber)'>needs-input</b> — 여러분의 입력을 기다림</li><li><b style='color:var(--ink-3)'>idle</b> — 실행 중인 것 없음</li></ul><p>상태는 패널별 컬러 점+라벨과 사이드바의 워크스페이스별 빨강/주황 배지로 표시됩니다. 앱이 백그라운드에 있을 때는 <b>macOS 알림</b>으로 손이 필요한 순간을 알려줍니다.</p><p>대기 중인 에이전트가 있으면 상단 바에 빨간 <b>&ldquo;대기 N&rdquo;</b> 필이 표시됩니다. <kbd>⌘J</kbd>(Windows는 <kbd>Ctrl+Alt+J</kbd>)를 누르거나 필·사이드바의 빨간 배지를 클릭하면 입력이 필요한 터미널로 바로 이동합니다 — 필요하면 워크스페이스도 자동 전환됩니다.</p><p>알림 범위는 <b>설정</b>에서 조절할 수 있습니다 — 입력 필요 + 작업 완료(기본), 입력 필요만, 끄기. &ldquo;입력 필요&rdquo; 알림을 클릭하면 앱이 앞으로 오면서 해당 터미널로 바로 이동합니다.</p>",
    ...
    "docs.diff.b": "<p><b>± 버튼</b>은 패널의 리뷰 패널을 엽니다: 추가/삭제 수가 있는 변경 파일 목록과 컬러 unified diff를 worktree 베이스 기준으로(에이전트 커밋 포함) 보여줍니다.</p><ul><li>한 번의 클릭으로 전체를 저장소에 <b>스쿼시 병합</b>합니다 — 충돌 시 안전하게 롤백됩니다.</li><li>또는 확인 후 전부 <b>폐기</b>합니다.</li><li>병합이 끝나면 확인 한 번으로 <b>정리</b>까지: worktree 제거, <code>bluedeck/*</code> 브랜치 삭제, 터미널을 워크스페이스 디렉터리로 복귀시킵니다.</li></ul>",
    ...
    "docs.shortcuts.b": "<p>앱 안에서 <kbd>⌘/</kbd>(Windows는 <kbd>Ctrl+Alt+/</kbd>)를 누르면 언제든 이 목록을 볼 수 있습니다.</p><table><thead><tr><th>동작</th><th>macOS</th><th>Windows</th></tr></thead><tbody><tr><td>1–9번 터미널로 포커스</td><td><kbd>⌘1</kbd>–<kbd>⌘9</kbd></td><td><kbd>Ctrl+Alt+1</kbd>–<kbd>9</kbd></td></tr><tr><td>입력 대기 중인 터미널로 이동</td><td><kbd>⌘J</kbd></td><td><kbd>Ctrl+Alt+J</kbd></td></tr><tr><td>커맨드 팔레트</td><td><kbd>⌘K</kbd></td><td><kbd>Ctrl+Alt+K</kbd></td></tr><tr><td>사이드바 토글</td><td><kbd>⌘B</kbd></td><td><kbd>Ctrl+B</kbd></td></tr><tr><td>단축키 도움말</td><td><kbd>⌘/</kbd></td><td><kbd>Ctrl+Alt+/</kbd></td></tr><tr><td>터미널 / 워크스페이스 이름 변경</td><td colspan='2'>이름 더블클릭</td></tr></tbody></table>",

    "docs.license.t": "라이선스 & Lite 모드",
    "docs.license.b": "<div class='callout'><span class='ci'>🎉</span><span><b>공개 베타:</b> <b>2026년 8월 31일</b>까지 모든 기능이 무료입니다 — 키가 필요 없습니다. 베타가 끝나면 모두에게 새 14일 트라이얼이 시작됩니다. 아래 플랜은 정식 출시부터 적용됩니다.</span></div><p>모든 설치는 <b>14일 풀 트라이얼</b>로 시작합니다(모든 기능, 신용카드 불필요). 트라이얼이 끝나면 BlueDeck는 <b>Lite 모드</b>로 전환됩니다:</p><ul><li><b>Lite(무료):</b> 코어 터미널 — 워크스페이스 1개 × 터미널 2개, 에이전트 실행, IME, 프롬프트 바, 레이아웃 복원.</li><li><b>Pro(라이선스):</b> 전체 멀티 에이전트 덱 — 상태 인식, worktree 격리, diff 리뷰, Agent Hub, 브로드캐스트, 자동 승인, 대화 이어가기.</li></ul><p><b>Settings → License</b>에서 키를 입력해 기기를 활성화합니다. 활성화는 저장되어 앱이 <b>무기한 오프라인</b>으로 동작하며, 백그라운드에서 온라인 재검증을 하고 &ldquo;이 기기 비활성화&rdquo; 시 좌석을 반환합니다. 유료 기능은 UI뿐 아니라 Rust 코어에서 강제됩니다.</p>",
    ...
    "docs.bugs.t": "버그 신고",
    "docs.bugs.b": "<p>문제를 발견하셨나요? 앱을 벗어나지 않고 바로 신고할 수 있습니다: <b>설정 → 지원 → 버그 신고</b>, 또는 커맨드 팔레트(<kbd>⌘K</kbd>)에서 실행하세요.</p><ul><li>제목과 내용을 쓰고, 원하면 <b>진단 정보</b>(버전, OS, 라이선스 등급, 워크스페이스/터미널 수)를 첨부합니다 — 보낼 내용을 그대로 미리 보여주며, 터미널 내용은 절대 포함하지 않습니다.</li><li>첫 제출 시 일회성 <b>기기 코드</b>로 GitHub 계정을 연결하며, 이후에는 클릭 한 번이면 됩니다. 신고는 <code>cslim0527/blue-deck-releases</code>에 <b>본인 계정 명의의 GitHub 이슈</b>로 등록되어 스레드에서 후속 문답이 가능합니다.</li><li>계정 연결이 부담스럽다면 <b>&ldquo;브라우저에서 작성&rdquo;</b>으로 내용이 미리 채워진 github.com 페이지를 열 수 있습니다.</li></ul>",
    "docs.faq.t": "자주 묻는 질문",
    "docs.faq.b": "<h3>지금은 무료인가요?</h3><p>네 — BlueDeck는 <b>공개 베타</b> 중이며 <b>2026년 8월 31일</b>까지 모든 기능이 무료입니다(키 불필요). 베타가 끝나면 모두에게 새 14일 트라이얼이 시작되고 Pro 가격이 적용됩니다.</p><h3>어떤 에이전트를 지원하나요?</h3><p>Claude Code, Codex, Gemini CLI, Aider를 자동 감지합니다. 다른 CLI도 패널에서 직접 실행할 수 있습니다.</p><h3>제 코드가 어딘가로 전송되나요?</h3><p>아니요. 터미널은 로컬에서 실행되고, Agent Hub MCP 서버는 <code>127.0.0.1</code>(로컬호스트)에만 바인딩됩니다. 에이전트는 일반 터미널에서와 동일하게 각자의 제공자와 통신합니다.</p><h3>오프라인에서 동작하나요?</h3><p>네. 라이선스가 활성화되면 로컬에 저장되어 오프라인으로 동작하며, 가능할 때 백그라운드에서만 온라인 재검증을 합니다.</p><h3>어떤 플랫폼을 지원하나요?</h3><p>Apple Silicon macOS와 x64 Windows이며, 둘 다 자동 업데이트됩니다.</p>",
  },
};
